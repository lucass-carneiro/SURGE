...
#  define SURGE_DEBUG_BANNER                                                                       \
    "\033[36m[Thread ID: {}]\033[m \033[1m\033[34mSURGE Debug Info:\033[m "
...
#define log_error(format, ...) fmt::print(SURGE_ERROR_BANNER format "\n", SURGE_TID_FUNCTION __VA_OPT__(, ) __VA_ARGS__)

#define log_info(format, ...) fmt::print(SURGE_INFO_BANNER format "\n", SURGE_TID_FUNCTION __VA_OPT__(, ) __VA_ARGS__)

#define log_warn(format, ...) fmt::print(SURGE_WARN_BANNER format "\n", SURGE_TID_FUNCTION __VA_OPT__(, ) __VA_ARGS__)

#ifdef SURGE_BUILD_TYPE_Debug
#  define log_debug(format, ...) fmt::print(SURGE_DEBUG_BANNER format "\n", SURGE_TID_FUNCTION __VA_OPT__(, ) __VA_ARGS__)
#else
