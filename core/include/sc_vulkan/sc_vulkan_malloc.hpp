...
auto create_memory_allocator(VkInstance instance, VkPhysicalDevice phys_dev, VkDevice logi_dev)
    -> Result<VmaAllocator>;

auto create_buffer(Context ctx, size_t size, VkBufferUsageFlags usage_flags,
                   VmaMemoryUsage memory_usage) -> Result<Buffer>;

auto file_to_buffer(Context ctx, const char *path, VkBufferUsageFlags usage_flags,
                    VmaMemoryUsage memory_usage) -> Result<Buffer>;
