...
enum class cast_error { target_type_too_small, negative_to_unsigned_undefined };

template <std::integral target_type, std::integral original_type>
constexpr inline auto safe_cast(original_type value) -> tl::expected<target_type, cast_error> {
  using namespace tl;
  ...
  if constexpr ((std::is_unsigned<original_type>::value && std::is_unsigned<target_type>::value)
    ...
    if (value <= std::numeric_limits<target_type>::max()) {
      ...
      log_all<log_event::error>(
          "Attempt to cast to a target type to small to hold the original value;");
      return unexpected(cast_error::target_type_too_small);
    }
  } else {
    log_all<log_event::error>("Casting a negative number to a unsigned type is undefined.");
