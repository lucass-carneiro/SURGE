...
static auto link_single_shader(GLuint shader_handle, bool destroy_shaders = true) noexcept
  ...
  log_info("Linking shader handle %u", shader_handle);
  ...
  if (is_linked == GL_FALSE) {
    ...
    log_info("Failed to link shader handles %u to create program:\n  %s", shader_handle,
             info_log.data());
    return tl::unexpected(surge::error::shader_link_error);
    ...
    if (destroy_shaders) {
      log_info("Destroying shaders handles %u", shader_handle);
      glDeleteShader(shader_handle);
    }

    log_info("Shader handle %u linked succesfully. Program handle: %u", shader_handle,
             program_handle_tmp);
...
auto surge::renderer::create_compute_shader(const char *compute_shader_path) noexcept
    -> tl::expected<GLuint, error> {
#if defined(SURGE_BUILD_TYPE_Profile) && defined(SURGE_ENABLE_TRACY)
  ZoneScopedN("surge::renderer::create_compute_shader");
#endif
...
void surge::renderer::dispatch_compute(GLuint program, GLuint x, GLuint y, GLuint z) noexcept {
  glUseProgram(program);
  glDispatchCompute(x, y, z);
}

void surge::renderer::dispatch_compute(GLuint program, GLuint x, GLuint y, GLuint z,
