...
void surge::show_main_gui_window(bool *open) noexcept {
  ...
  auto &window{global_engine_window::get()};
  const auto &config{window.get_config()};
  ...
  SetNextWindowSize(ImVec2(config->window_width / 2, config->window_height / 2),
                    ImGuiCond_FirstUseEver);

  // Begin window
  if (!Begin("SURGE Debug Window", open, ImGuiWindowFlags_MenuBar)) {
    End();
    return;
  }
