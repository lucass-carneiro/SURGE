...
/*
 * This is for generating a hash from the "unique" ID produced by concatenating the file name and
 * line into a string. Hash generation is done at compile time
 */
...
template <int size, int idx = 0, class dummy = void> struct MM {
  static constexpr auto crc32(const char *str, surge::u32 prev_crc = 0xFFFFFFFF) -> surge::u32 {
    // NOLINTNEXTLINE
...
template <int size, class dummy> struct MM<size, size, dummy> {
  static constexpr auto crc32(const char *, surge::u32 prev_crc = 0xFFFFFFFF) -> surge::u32 {
...
#define U_ID                                                                                       \
  (MM<sizeof("gui_id" __FILE__ "_" STRINGIZE(__LINE__)) - 1>::crc32("gui_id" __FILE__              \
